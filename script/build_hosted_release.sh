...
if ! docker info >/dev/null 2>&1; then
  echo "Docker Desktop must be running to build the linux/amd64 hosted release" >&2
  exit 1
fi

OUTPUT_PARENT="$ROOT_DIRECTORY/dist/hosted"
...
docker buildx build \
  ...
  --file "$ROOT_DIRECTORY/deploy/hosted/Dockerfile" \
  --target release \
