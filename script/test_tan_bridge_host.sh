...
docker run --rm \
  ...
  "${idf_image}" \
  bash -lc 'cmake -E remove_directory host-tests/build && cmake -S host-tests -B host-tests/build -G Ninja && cmake --build host-tests/build && ctest --test-dir host-tests/build --output-on-failure'
